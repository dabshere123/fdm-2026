    ...
    return(
      <div style={{...S.root,background:'#1a0a00'}}>
        <div style={S.hdr}>
          ...
          <div style={{fontSize:15,fontWeight:900,color:'#fcd34d'}}>🚨 PA Script Ready</div>
        </div>
        <div style={{padding:'16px',display:'flex',flexDirection:'column',gap:12}}>
          ...
          <div style={{background:'rgba(0,0,0,0.5)',border:'2px solid rgba(234,179,8,0.6)',borderRadius:12,padding:'16px'}}>
            ...
            <div style={{fontSize:14,color:'#f1f5f9',lineHeight:1.9,whiteSpace:'pre-line',fontFamily:'monospace'}}>{script}</div>
          </div>
          <button style={{padding:'14px',borderRadius:12,border:'none',background:'rgba(234,179,8,0.15)',color:'#fcd34d',fontSize:14,fontWeight:700,cursor:'pointer'}} onClick={function(){
            if(navigator.clipboard){navigator.clipboard.writeText(script);}
            alert('Script copied!');
          }}>📋 Copy Script</button>
          <button style={{padding:'14px',borderRadius:12,border:'1px solid rgba(255,255,255,0.1)',background:'rgba(255,255,255,0.04)',color:'#94a3b8',fontSize:14,fontWeight:700,cursor:'pointer'}} onClick={onBack}>Done</button>
        </div>
      </div>
    );
  }
...
function LFView({user,onBack}){
  ...
  const [currentLoc,setCurrentLoc]=React.useState(user.location||'');
  ...
  function compressPhoto(file,cb){
    const SAFE_LIMIT=70000; // stay well under Airtable's long-text practical limit
    ...
    r.onload=e=>{
      ...
      img.onload=()=>{
        ...
        // Progressively shrink quality/size until it fits, never truncate the result
        ...
        cb(result.length>SAFE_LIMIT?null:result); // null = genuinely couldn't compress small enough
      };
  ...
  async function submit(){
    if(!photo||!description||!foundAt) return;
    setSending(true);setSaveError('');
    try{
      const res=await fetch(`${API}/submit-lost-found`,{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({description,foundAt,currentLocation:currentLoc||foundAt,dayFound:day,foundBy:user.name,role:user.role,photoData:photo})});
      const data=await res.json();
      setItemNumber(data.itemNumber||'???');
      if(data.airtableError) setSaveError(data.airtableError);
      setStep('confirm');
    }catch(e){setSaveError(e.message);setStep('confirm');}
    setSending(false);
  }

  // CONFIRM POPUP
  if(step==='confirm') return(
    <div style={{...S.root,display:'flex',alignItems:'center',justifyContent:'center',padding:'24px 20px'}}>
      <div style={{background:'rgba(13,13,26,0.97)',border:'2px solid rgba(249,115,22,0.5)',borderRadius:20,padding:'28px 20px',width:'100%',maxWidth:380,display:'flex',flexDirection:'column',alignItems:'center',gap:16,textAlign:'center'}}>
        <div style={{fontSize:56}}>{'📦'}</div>
        <div style={{fontSize:22,fontWeight:900,color:'#f1f5f9'}}>{'Item Logged!'}</div>
        <div style={{background:'rgba(249,115,22,0.12)',border:'2px solid rgba(249,115,22,0.5)',borderRadius:14,padding:'16px 20px',width:'100%'}}>
          <div style={{fontSize:12,fontWeight:700,color:'#94a3b8',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:6}}>{'Tag Number'}</div>
          <div style={{fontSize:32,fontWeight:900,color:'#fb923c',letterSpacing:'0.04em'}}>{itemNumber}</div>
        </div>
        {saveError&&<div style={{background:'rgba(239,68,68,0.1)',border:'1px solid rgba(239,68,68,0.3)',borderRadius:10,padding:'10px 14px',fontSize:12,color:'#fca5a5',width:'100%'}}>{'⚠️ Item tagged but NOT saved to lookup. Error: '+saveError.slice(0,100)}</div>}
        <div style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:12,padding:'14px 16px',width:'100%',textAlign:'left'}}>
          <div style={{fontSize:13,fontWeight:800,color:'#f1f5f9',marginBottom:8}}>{'📝 Next Steps:'}</div>
          <div style={{fontSize:13,color:'#94a3b8',lineHeight:1.7}}>
            {'1. Write '}<strong style={{color:'#fb923c'}}>{itemNumber}</strong>{' on a piece of tape\n2. Attach tag to the item\n3. Place item in the Lost & Found box\n4. Devin will collect it each night'}
          </div>
        </div>
        <button style={{width:'100%',padding:'14px',borderRadius:12,border:'none',background:'linear-gradient(135deg,#16a34a,#15803d)',color:'#fff',fontSize:15,fontWeight:900,cursor:'pointer'}} onClick={()=>{setStep('done');}}>{'✅ Done — Item Boxed'}</button>
      </div>
    </div>
  );
  ...
  if(step==='done') return(
    <div style={{...S.root,display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:16,padding:32,textAlign:'center'}}>
      ...
      <div style={{fontSize:18,fontWeight:900,color:'#4ade80'}}>{'Item in the Box!'}</div>
      <div style={{fontSize:13,color:'#64748b'}}>{'Tag #'+itemNumber+' · Devin collects nightly'}</div>
  ...
  // MAIN FORM
  const canSubmit=photo&&description.trim()&&foundAt.trim()&&!sending;
  return(
    <div style={S.root}>
      <div style={S.hdr}>
        <BackBtn onBack={onBack}/>
        <div style={{fontSize:15,fontWeight:900}}>{'📋 Log Lost Item'}</div>
      </div>
      <div style={{padding:'16px',display:'flex',flexDirection:'column',gap:14,overflowY:'auto',paddingBottom:32}}>

        {/* PHOTO — REQUIRED FIRST */}
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          <div style={{fontSize:11,fontWeight:900,color:photo?'#4ade80':'#f97316',textTransform:'uppercase',letterSpacing:'0.08em'}}>
          ...
          {photo?(
            <div style={{position:'relative',borderRadius:12,overflow:'hidden',border:'2px solid rgba(34,197,94,0.4)'}}>
              <img src={photo} alt="item" style={{width:'100%',maxHeight:220,objectFit:'cover',display:'block'}}/>
              <button style={{position:'absolute',top:8,right:8,background:'rgba(0,0,0,0.7)',border:'1px solid rgba(255,255,255,0.2)',borderRadius:8,color:'#f1f5f9',padding:'6px 10px',fontSize:12,cursor:'pointer'}} onClick={()=>setPhoto(null)}>{'Retake'}</button>
            </div>
          ):(
            <div style={{background:'rgba(249,115,22,0.06)',border:'2px dashed rgba(249,115,22,0.4)',borderRadius:12,padding:'32px 20px',display:'flex',flexDirection:'column',alignItems:'center',gap:10,cursor:'pointer'}} onClick={()=>photoRef.current?.click()}>
              <div style={{fontSize:40}}>{'📷'}</div>
              <div style={{fontSize:14,fontWeight:700,color:'#fb923c'}}>{'Tap to Take Photo'}</div>
              <div style={{fontSize:12,color:'#64748b'}}>{'Required before submitting'}</div>
            </div>
          )}
          <input ref={photoRef} type="file" accept="image/*" capture="environment" style={{display:'none'}} onChange={e=>{if(e.target.files[0]) compressPhoto(e.target.files[0],p=>{setPhoto(p);if(!p)alert('That photo could not be compressed small enough to save. Please try a different photo.');});}}/>
        </div>

        {/* DESCRIPTION */}
        <div>
          <div style={{fontSize:11,fontWeight:900,color:'#94a3b8',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:6}}>{'Description *'}</div>
          <textarea style={{width:'100%',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:10,padding:'13px',color:'#f1f5f9',fontSize:15,fontFamily:'inherit',outline:'none',resize:'none',minHeight:80}} placeholder="e.g. Black iPhone 14, cracked screen · Blue Hydro Flask, green lid · Car keys, Toyota fob" value={description} onChange={e=>setDescription(e.target.value)}/>
        </div>

        {/* WHERE FOUND */}
        <div>
          <div style={{fontSize:11,fontWeight:900,color:'#94a3b8',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:6}}>{'Where Was It Found? *'}</div>
          <input style={{width:'100%',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:10,padding:'13px',color:'#f1f5f9',fontSize:15,fontFamily:'inherit',outline:'none'}} placeholder="e.g. Moon Bar near east exit, Sun Stage left side" value={foundAt} onChange={e=>setFoundAt(e.target.value)}/>
        </div>

        {/* DAY */}
        <div>
          <div style={{fontSize:11,fontWeight:900,color:'#94a3b8',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:8}}>{'Day Found'}</div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
            {DAYS.map(d=>(
              <button key={d} style={{padding:'12px',borderRadius:10,border:day===d?'2px solid #fb923c':'1px solid rgba(255,255,255,0.1)',background:day===d?'rgba(249,115,22,0.15)':'rgba(255,255,255,0.04)',color:day===d?'#fb923c':'#94a3b8',fontSize:14,fontWeight:700,cursor:'pointer'}} onClick={()=>setDay(day===d?'':d)}>{d}</button>
            ))}
          </div>
        </div>
        ...
        {/* CURRENT LOCATION */}
        <div>
          <div style={{fontSize:11,fontWeight:900,color:'#94a3b8',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:6}}>{'Where Is It Now?'}</div>
          <input style={{width:'100%',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:10,padding:'13px',color:'#f1f5f9',fontSize:15,fontFamily:'inherit',outline:'none'}} placeholder="e.g. With me at Moon Bar, Admin Tent" value={currentLoc} onChange={e=>setCurrentLoc(e.target.value)}/>
        </div>

        {/* SUBMIT */}
        {!photo&&<div style={{background:'rgba(249,115,22,0.07)',border:'1px solid rgba(249,115,22,0.2)',borderRadius:10,padding:'10px 14px',fontSize:12,color:'#fb923c',textAlign:'center'}}>{'📷 Take a photo to enable submit'}</div>}
        <button style={{padding:'18px',borderRadius:12,border:'none',background:canSubmit?'linear-gradient(135deg,#ea580c,#c2410c)':'rgba(255,255,255,0.06)',color:canSubmit?'#fff':'#475569',fontSize:16,fontWeight:900,cursor:canSubmit?'pointer':'not-allowed'}}
          disabled={!canSubmit}
          onClick={submit}>
          {sending?'Saving...':'📦 Submit & Get Tag Number'}
        </button>
      </div>
    </div>
  );
...
// ── HOME ─────────────────────────────────────────────────────
function FoundLostChildView({user,onBack}){
  const [step,setStep]=React.useState('form');
  const [childName,setChildName]=React.useState('');
  const [childAge,setChildAge]=React.useState('');
  const [childHair,setChildHair]=React.useState('');
  const [childClothing,setChildClothing]=React.useState('');
  const [childLocation,setChildLocation]=React.useState(user.location||'');
  const [guardianName,setGuardianName]=React.useState('');
  const [sending,setSending]=React.useState(false);

  const canSubmit=childAge.trim()&&childClothing.trim()&&childLocation.trim();

  async function submit(){
    if(!canSubmit) return;
    setSending(true);
    const desc=[
      childName?`Child: ${childName}`:'Unknown name',
      childAge?`Age: ${childAge}`:'',
      childHair?`Hair: ${childHair}`:'',
  ...
  if(step==='script'){
    const nameStr=childName?childName:'an unidentified child';
    const ageStr=childAge?`approximately ${childAge} years old`:'';
    const hairStr=childHair?`, ${childHair} hair`:'';
    const clothStr=childClothing?`, wearing ${childClothing}`:'';
    const desc2=[ageStr,hairStr,clothStr].filter(Boolean).join('');
...
Thank you, and enjoy the festival.`;
    return(
      <div style={{...S.root,background:'#0a1a0a'}}>
        <div style={S.hdr}>
          <BackBtn onBack={onBack}/>
          <div style={{fontSize:15,fontWeight:900,color:'#4ade80'}}>🧒✅ PA Script Ready</div>
        </div>
        <div style={{padding:'16px',display:'flex',flexDirection:'column',gap:12}}>
          <div style={{background:'rgba(34,197,94,0.1)',border:'2px solid rgba(34,197,94,0.4)',borderRadius:12,padding:'14px'}}>
            <div style={{fontSize:11,fontWeight:900,color:'#4ade80',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:4}}>✅ Alert sent to all staff</div>
            <div style={{fontSize:12,color:'#86efac'}}>Hub notified · Broadcast sent</div>
          </div>
          <div style={{background:'rgba(0,0,0,0.4)',border:'2px solid rgba(234,179,8,0.5)',borderRadius:12,padding:'16px'}}>
            <div style={{fontSize:11,fontWeight:900,color:'#fcd34d',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:10}}>📢 STAGE MANAGER — READ OVER PA NOW</div>
            <div style={{fontSize:14,color:'#f1f5f9',lineHeight:1.8,whiteSpace:'pre-line',fontFamily:'monospace'}}>{script}</div>
          </div>
          <button style={{padding:'14px',borderRadius:12,border:'none',background:'rgba(234,179,8,0.15)',color:'#fcd34d',fontSize:14,fontWeight:700,cursor:'pointer'}} onClick={()=>{
            if(navigator.clipboard){navigator.clipboard.writeText(script);}
  ...
  return(
    <div style={S.root}>
      <div style={S.hdr}>
        <BackBtn onBack={onBack}/>
        <div style={{fontSize:15,fontWeight:900,color:'#4ade80'}}>🧒✅ Found Child</div>
      </div>
      <div style={{padding:'16px',display:'flex',flexDirection:'column',gap:12}}>
        <div style={{background:'rgba(34,197,94,0.08)',border:'1px solid rgba(34,197,94,0.2)',borderRadius:10,padding:'12px 14px',fontSize:12,color:'#86efac',lineHeight:1.5}}>
          Fill in as much as you know. This alerts all staff and generates a PA announcement script.
        </div>
        <div><div style={{fontSize:11,fontWeight:800,color:'#94a3b8',textTransform:'uppercase',marginBottom:6}}>Child's Name (if known)</div>
          <input style={{width:'100%',padding:'13px',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:10,color:'#f1f5f9',fontSize:16,fontFamily:'inherit',outline:'none'}} placeholder="e.g. Emma" value={childName} onChange={function(e){setChildName(e.target.value);}}/></div>
        <div><div style={{fontSize:11,fontWeight:800,color:'#94a3b8',textTransform:'uppercase',marginBottom:6}}>Approx. Age *</div>
          <input style={{width:'100%',padding:'13px',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:10,color:'#f1f5f9',fontSize:16,fontFamily:'inherit',outline:'none'}} placeholder="e.g. 5 years old" value={childAge} onChange={function(e){setChildAge(e.target.value);}}/></div>
        <div><div style={{fontSize:11,fontWeight:800,color:'#94a3b8',textTransform:'uppercase',marginBottom:6}}>Hair Color / Description</div>
          <input style={{width:'100%',padding:'13px',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:10,color:'#f1f5f9',fontSize:16,fontFamily:'inherit',outline:'none'}} placeholder="e.g. Brown, curly" value={childHair} onChange={function(e){setChildHair(e.target.value);}}/></div>
        <div><div style={{fontSize:11,fontWeight:800,color:'#94a3b8',textTransform:'uppercase',marginBottom:6}}>Clothing Description *</div>
          <input style={{width:'100%',padding:'13px',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:10,color:'#f1f5f9',fontSize:16,fontFamily:'inherit',outline:'none'}} placeholder="e.g. Red shirt, blue shorts" value={childClothing} onChange={function(e){setChildClothing(e.target.value);}}/></div>
        <div><div style={{fontSize:11,fontWeight:800,color:'#94a3b8',textTransform:'uppercase',marginBottom:6}}>Your Location (where child is now) *</div>
          <input style={{width:'100%',padding:'13px',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:10,color:'#f1f5f9',fontSize:16,fontFamily:'inherit',outline:'none'}} placeholder="e.g. Moon Stage, Medical Tent" value={childLocation} onChange={function(e){setChildLocation(e.target.value);}}/></div>
        <div><div style={{fontSize:11,fontWeight:800,color:'#94a3b8',textTransform:'uppercase',marginBottom:6}}>Guardian Name (if known)</div>
          <input style={{width:'100%',padding:'13px',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:10,color:'#f1f5f9',fontSize:16,fontFamily:'inherit',outline:'none'}} placeholder="e.g. Jane Smith" value={guardianName} onChange={function(e){setGuardianName(e.target.value);}}/></div>
        <button style={{padding:'18px',borderRadius:12,border:'none',background:canSubmit&&!sending?'linear-gradient(135deg,#16a34a,#15803d)':'rgba(255,255,255,0.06)',color:canSubmit&&!sending?'#fff':'#475569',fontSize:16,fontWeight:900,cursor:canSubmit&&!sending?'pointer':'not-allowed'}} disabled={!canSubmit||sending} onClick={submit}>
          {sending?'Sending Alert...':'🧒✅ Alert Staff + Get PA Script'}
        </button>
      </div>
    </div>
  );
    ...
    <div style={S.root}>
      ...
      <div style={S.body}>
        ...
        <button style={{width:'100%',padding:'20px 16px',borderRadius:14,border:'2px solid rgba(234,179,8,0.6)',background:'linear-gradient(135deg,rgba(202,138,4,0.2),rgba(161,98,7,0.1))',cursor:'pointer',textAlign:'left',display:'flex',alignItems:'center',gap:14}} onClick={()=>{setLostChildMenuOpen(false);goCall('lost_child');}}>
          ...
          <div>
            <div style={{fontSize:16,fontWeight:900,color:'#fcd34d'}}>REPORTED MISSING</div>
        ...
        <button style={{width:'100%',padding:'20px 16px',borderRadius:14,border:'2px solid rgba(34,197,94,0.5)',background:'linear-gradient(135deg,rgba(22,163,74,0.15),rgba(16,185,129,0.1))',cursor:'pointer',textAlign:'left',display:'flex',alignItems:'center',gap:14}} onClick={()=>setFoundChildView(true)}>
          <div style={{fontSize:34}}>🧒✅</div>
          <div>
            <div style={{fontSize:16,fontWeight:900,color:'#4ade80'}}>LOST — NOT REPORTED</div>
  ...
  if(DORMANT){
    return (
      <div style={{minHeight:"100vh",background:"#0a0f1e",color:"#f1f5f9",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px",textAlign:"center",fontFamily:"-apple-system,sans-serif"}}>
        ...
        <div style={{fontSize:15,color:"#94a3b8",maxWidth:400,lineHeight:1.6}}>Thanks for an incredible festival. The Worker App is now closed for the season — see you in 2027!</div>
      </div>
    );
  }
