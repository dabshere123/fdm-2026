...
function HubApp({onBack}){
    ...
    <div style={S.root}><Bg/><div style={S.panel}>
      ...
      {!alertView?(
        ...
        return(<div style={S.cWrap}>
          ...
          {/* STAGE/AREA TARGETING — manual tabs */}
          {t.id==="weather_imminent"&&(
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              <label style={{fontSize:12,color:"#64748b",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em"}}>Areas Affected (select all that apply)</label>
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                {["All Areas","Moon Stage","Sun Stage","Lagniappe","Lafayette","Cabaret","Family Fête"].map(area=>{
                  const sel=(alertFields._targetAreas||[]).includes(area);
                  return(
                    <button key={area} style={{padding:"10px 14px",borderRadius:8,border:`2px solid ${sel?"rgba(245,158,11,0.7)":"rgba(255,255,255,0.1)"}`,background:sel?"rgba(245,158,11,0.15)":"rgba(255,255,255,0.03)",color:sel?"#fbbf24":"#64748b",fontSize:13,fontWeight:sel?800:400,cursor:"pointer"}}
                      onClick={()=>{
                        const cur=alertFields._targetAreas||[];
                        if(area==="All Areas"){
                          // Toggle all
                          const all=["All Areas","Moon Stage","Sun Stage","Lagniappe","Lafayette","Cabaret","Family Fête"];
                          setAlertFields(p=>({...p,_targetAreas:sel?[]:all}));
                        } else {
                          const next=sel?cur.filter(x=>x!==area):[...cur,area];
                          setAlertFields(p=>({...p,_targetAreas:next}));
                        }
                        setEditedMsg("");
                      }}>
                      {sel?"✓ ":""}{area}
              ...
              {(alertFields._targetAreas||[]).length>0&&(
                <div style={{fontSize:12,color:"#f59e0b",background:"rgba(245,158,11,0.08)",borderRadius:8,padding:"8px 10px",border:"1px solid rgba(245,158,11,0.2)"}}>
                  📍 Notifying: {(alertFields._targetAreas||[]).join(", ")}
                </div>
              )}
            </div>
          )}
  ...
  const targetAreas=alertFields._targetAreas||[];
  const weatherTypes=alertFields._weatherTypes||[];
  const isHeavy=weatherTypes.some(w=>["Thunderstorm","Severe Thunderstorm","High Winds","Tornado Watch","Tornado Warning"].includes(w));
  const isRain=weatherTypes.some(w=>["Rain Storm","Torrential Rain / Downpour"].includes(w));
  let areasStr="";
  if(t.id==="weather_imminent"){
    if(isHeavy) areasStr="\nAFFECTED: ALL AREAS — Full Shutdown Protocol";
    else if(isRain) areasStr="\nAFFECTED: Lagniappe, Lafayette, Cabaret, Sun Stage, Family Fête\nMoon Stage remains open (tent)";
    else if(targetAreas.length>0) areasStr="\nAFFECTED: "+targetAreas.join(", ");
  }
      ...
      <div style={S.cWrap}>
        ...
        <button style={{...S.sendBtn,background:"linear-gradient(135deg,#6366f1,#4f46e5)"}}
          onClick={async()=>{
            ...
            // Try SMS (non-fatal if fails)
            fetch("/.netlify/functions/send-sms",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:"+16082289692",message:summary})}).catch(e=>console.log("SMS error:",e));
