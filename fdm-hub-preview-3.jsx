  ...
  if(view==="alert") return(
    <div style={S.root}><Bg/><div style={S.panel}>
      ...
      {!alertView?(
        ...
        const msgBase=(t?.defaultMsg||"").replace("[REASON]",selectedReason||"[select reason above]").replace("[WEATHER_TYPE]",alertFields._weatherType||"[select weather type above]");
        ...
        return(<div style={S.cWrap}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            ...
            <div style={{display:"flex",gap:8}}>
              {[{id:"sms_all",label:"All Staff & Vendors"}].map(opt=>{
            ...
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                {["Thunderstorm","High Winds","Tornado Warning","Flash Flood","Lightning","Severe Storm"].map(w=>{
                  const sel=alertFields._weatherType===w;
                  return(
                    <button key={w} style={{padding:"10px 14px",borderRadius:8,border:`1px solid ${sel?"rgba(239,68,68,0.6)":"rgba(255,255,255,0.1)"}`,background:sel?"rgba(239,68,68,0.15)":"rgba(255,255,255,0.03)",color:sel?"#ef4444":"#64748b",fontSize:14,fontWeight:sel?700:400,cursor:"pointer"}}
                      onClick={()=>{setAlertFields(p=>({...p,_weatherType:w}));setEditedMsg("");}}>
                      {sel?"✓ ":""}{w}
                    </button>
                  );
                })}
