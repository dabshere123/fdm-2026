...
const BCAST_GROUP_ROLES={
  all_staff:["admin","a1","a2","m1","med 1","med unit 1","m2","med 2","med unit 2","sbl","sbr","cb","ff","mb1","mb2","lb","lgb","eteb","sb","csb","fsf","ms","laf","lag","mt","mkt","f1","f2","f3","f4","fip","youth organizer","sun bar left help","vend","oc1","oc2","oc3","oc4"],
  admin:["admin","a1","a2"],
  medical:["m1","med 1","med unit 1","m2","med 2","med unit 2"],
  bars:["mb1","mb2","sbl","sbr","lb","lgb","eteb","sb","csb","sun bar left help"],
  stages:["ff","fsf","ms","laf","lag","mt","cb"],
  misc:["fip","youth organizer","mkt","20th"],
  eec:["eteb"],
  prize_wheel:["mt","mkt","prize wheel"],
};
...
const BCAST_LOC_ROLES={
  "Admin":["admin","a1","a2"],
  "Med 1":["m1","med 1","med unit 1"],
  "Med 2":["m2","med 2","med unit 2"],
  "Moon Stage":["ms"],
  "Sun Stage":["fsf","ss"],
  "Lagniappe Stage":["lag","lagniappe stage"],
  "Lafayette Stage":["laf","lafayette stage"],
  "Family Fete Stage":["ff","family fete"],
  "Cabaret Stage":["cb","cabaret"],
  "Moon Bar":["mb1","mb2"],
  "Sun Left Bar":["sbl","sun bar left help"],
  "Sun Right Bar":["sbr"],
  "Cabaret Bar":["csb"],
  "Lafayette Bar":["lb"],
  "Lagniappe Bar":["lgb"],
  "Family Fete Bar":["sb"],
  "Everything Else Cafe":["eteb"],
  "Prize Wheel":["mt","mkt"],
};
// Individual locations → Festival Chat channels
const BCAST_LOC_CHANNELS={
  "Admin":"Admin","Med 1":"AdminMed","Med 2":"AdminMed",
  "Moon Stage":"MoonST","Sun Stage":"SunST","Lagniappe Stage":"LagST",
  "Lafayette Stage":"LafST","Family Fete Stage":"FamST","Cabaret Stage":"CabST",
  "Moon Bar":"MoonBar","Sun Left Bar":"SunBarL","Sun Right Bar":"SunBarR",
  "Cabaret Bar":"CabBar","Lafayette Bar":"LafBar","Lagniappe Bar":"LagBar",
  "Family Fete Bar":"FamilyBar","Everything Else Cafe":"EverythingBar","Prize Wheel":"AllStaff",
};
const BCAST_WEATHER_TYPES=["Thunderstorm","Severe Thunderstorm","Rain Storm","Tornado Watch","Severe Thunderstorm Watch","Tornado Warning","Custom"];
const BCAST_GROUPS=[
...
const BCAST_LOCATIONS=["Admin","Med 1","Med 2","Moon Stage","Sun Stage","Lagniappe Stage","Lafayette Stage","Family Fete Stage","Cabaret Stage","Moon Bar","Sun Left Bar","Sun Right Bar","Cabaret Bar","Lafayette Bar","Lagniappe Bar","Family Fete Bar","Everything Else Cafe","Prize Wheel"];
...
Madison Fire/EMS INBOUND — McPike Park`;
    ...
    <div style={{display:"flex",flexDirection:"column",flex:1,overflowY:"auto"}}>
      ...
      <div style={{padding:"4px 16px 4px"}}>
        <button style={{width:"100%",padding:"16px",borderRadius:14,border:"2px solid rgba(219,39,119,0.7)",background:"linear-gradient(135deg,rgba(190,24,93,0.25),rgba(157,23,77,0.15))",cursor:"pointer",display:"flex",alignItems:"center",gap:14,boxShadow:"0 0 16px rgba(190,24,93,0.2)"}} onClick={()=>{setNewCallType("walk_in");setNewCallLocation("");setNewCallProblem("");setNewCallView(true);}}>
          <span style={{fontSize:26}}>🚶</span>
          <div style={{textAlign:"left"}}>
            <div style={{fontSize:16,fontWeight:900,color:"#fbcfe8"}}>WALK-IN PATIENT</div>
            <div style={{fontSize:11,color:"#e2e8f0",marginTop:2}}>Patient walked in · alerts med units</div>
          </div>
        </button>
      </div>
      ...
      <div style={{padding:"12px 16px",display:"flex",flexDirection:"column",gap:8,flex:1,overflowY:"auto"}}>
        ...
        {tab==="mycalls"&&(
          <>
            {myActive.length===0&&<div style={{textAlign:"center",color:"#e2e8f0",fontSize:13,padding:"24px 0"}}>No calls assigned to {role}</div>}
            {myActive.map(call=>{
        ...
        {tab==="unassigned"&&(
          <>
            {unassigned.length===0&&<div style={{textAlign:"center",color:"#e2e8f0",fontSize:13,padding:"24px 0"}}>No unassigned calls</div>}
            {unassigned.map(call=>{
              const c=CALL_COLORS[call.type]||CALL_COLORS.medical;
              return(
        ...
        {tab==="lf"&&(
          <>
            ...
            {(lfItems||[]).filter(i=>i.status!=="Claimed").length===0&&<div style={{textAlign:"center",color:"#e2e8f0",fontSize:13,padding:"20px 0"}}>No active L&F items</div>}
    ...
    <div style={{...S.root,display:"flex",flexDirection:"column",height:"100vh",maxHeight:"100vh"}}>
      <div style={S.panel}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"12px 16px",display:"flex",flexDirection:"column",gap:8,minHeight:0}}>
          {messages.length===0&&<div style={{textAlign:"center",color:"#e2e8f0",fontSize:13,padding:"24px 0"}}>No messages yet</div>}
          {messages.map(msg=>{
            ...
            return(
              <div key={msg.id} style={{display:"flex",flexDirection:"column",alignItems:isMe?"flex-end":"flex-start",gap:2}}>
                ...
                {isMe&&<div style={{fontSize:10,color:"#e2e8f0",marginRight:4}}>{time}</div>}
              </div>
            );
          })}
          <div ref={msgEnd}/>
            ...
            {alertFields._recipMode==="locations"&&(
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                {BCAST_LOCATIONS.map(loc=>{
                  ...
                  return(<button key={loc} style={{padding:"8px 14px",borderRadius:8,border:`1px solid ${sel?"rgba(16,185,129,0.6)":"rgba(255,255,255,0.1)"}`,background:sel?"rgba(16,185,129,0.12)":"rgba(255,255,255,0.03)",color:sel?"#6ee7b7":"#64748b",fontSize:12,fontWeight:sel?700:400,cursor:"pointer"}} onClick={()=>{
                    const cur=alertFields._recipLocs||[];
                    const next=sel?cur.filter(x=>x!==loc):[...cur,loc];
                    setAlertFields(p=>({...p,_recipLocs:next}));
                  }}>{sel?"✓ ":""}{loc}</button>);
                })}
              </div>
            )}
...
Reply YES to acknowledge.`
  ...
  if(mpdRequestView) return(
    <div style={S.root}><Bg/><div style={S.panel}>
      ...
      <div style={{padding:"20px 16px",display:"flex",flexDirection:"column",gap:14,overflowY:"auto"}}>
        <div style={{fontSize:13,color:"#94a3b8",lineHeight:1.6}}>
          SMS + Voice call to all officers currently set to <strong style={{color:"#4ade80"}}>Online</strong>. They reply ACK to confirm.
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          <div style={{fontSize:11,fontWeight:800,color:"#94a3b8",textTransform:"uppercase"}}>Location *</div>
          <input style={{width:"100%",padding:"13px",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,color:"#f1f5f9",fontSize:16,fontFamily:"inherit",outline:"none"}} placeholder="e.g. Sun Stage, Moon Bar" value={mpdLocation} onChange={function(e){setMpdLocation(e.target.value);}}/>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          <div style={{fontSize:11,fontWeight:800,color:"#94a3b8",textTransform:"uppercase"}}>Situation *</div>
          <textarea style={{width:"100%",padding:"13px",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,color:"#f1f5f9",fontSize:15,fontFamily:"inherit",outline:"none",resize:"none",minHeight:90}} placeholder="Describe the situation..." value={mpdSituation} onChange={function(e){setMpdSituation(e.target.value);}}/>
        </div>
        {mpdResult ? (
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            <div style={{background:"rgba(34,197,94,0.12)",border:"1px solid rgba(34,197,94,0.4)",borderRadius:12,padding:"14px",color:"#4ade80",fontWeight:700,fontSize:13}}>
              {"✅ "+(mpdResult.notified||0)+" officer"+(mpdResult.notified!==1?"s":"")+" notified — awaiting ACK"}
            </div>
            <button style={{padding:"13px",borderRadius:12,border:"2px solid rgba(239,68,68,0.5)",background:"rgba(239,68,68,0.08)",color:"#fca5a5",fontSize:14,fontWeight:800,cursor:"pointer"}} onClick={function(){
              var reason=window.prompt("Reason for disregard (optional):")||"";
              fetch("/.netlify/functions/disregard-mpd",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({reason:reason})}).then(function(r){return r.json();}).then(function(d){alert("Disregard sent to "+(d.notified||0)+" officer"+(d.notified!==1?"s":"")+".");setMpdResult(null);setMpdLocation("");setMpdSituation("");setMpdRequestView(false);});
            }}>{"🚫 Disregard — Cancel MPD Request"}</button>
          </div>
        ) : (
          <button style={{padding:"18px",borderRadius:12,border:"none",background:mpdLocation.trim()&&mpdSituation.trim()&&!mpdSending?"linear-gradient(135deg,rgba(37,99,235,0.9),rgba(29,78,216,0.9))":"rgba(255,255,255,0.06)",color:mpdLocation.trim()&&mpdSituation.trim()&&!mpdSending?"#fff":"#475569",fontSize:16,fontWeight:900,cursor:mpdLocation.trim()&&mpdSituation.trim()&&!mpdSending?"pointer":"not-allowed"}} disabled={!mpdLocation.trim()||!mpdSituation.trim()||mpdSending} onClick={function(){
            setMpdSending(true);
            fetch("/.netlify/functions/request-mpd",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({location:mpdLocation,situation:mpdSituation,requestedBy:role,callType:"security"})}).then(function(r){return r.json();}).then(function(d){setMpdResult(d);setMpdSending(false);}).catch(function(e){alert("Error: "+e.message);setMpdSending(false);});
          }}>{mpdSending?"Sending...":"🚔 Send SMS + Voice to All Officers"}</button>
        )}
        <button style={{padding:"12px",borderRadius:12,border:"1px solid rgba(255,255,255,0.1)",background:"rgba(255,255,255,0.05)",color:"#94a3b8",fontSize:14,fontWeight:700,cursor:"pointer"}} onClick={function(){setMpdRequestView(false);setMpdResult(null);setMpdLocation("");setMpdSituation("");}}>{"← Back"}</button>
      </div>
    </div></div>
  );
  ...
  if(view==="stafflist") return(
    <div style={S.root}><Bg/><div style={S.panel}>
      ...
      <div style={{flex:1,overflowY:"auto"}}>
        ...
        {staffList.length===0&&<div style={{textAlign:"center",color:"#e2e8f0",padding:30}}>No staff loaded</div>}
        {staffList.map((s,i)=>(
  ...
  if(view==="sendonboarding") return(
    <div style={{...S.root,position:"relative"}}><Bg/><div style={{...S.panel,overflowY:"auto"}}>
      ...
      <div style={{padding:"16px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div>
          <div style={{fontSize:11,fontWeight:800,color:"#94a3b8",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6}}>{"Full Name *"}</div>
          <input style={{width:"100%",padding:"13px 14px",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,color:"#f1f5f9",fontSize:16,fontFamily:"inherit",outline:"none"}} placeholder="e.g. Bryan Thornton" value={obName} onChange={e=>setObName(e.target.value)}/>
        </div>

        <div>
          <div style={{fontSize:11,fontWeight:800,color:"#94a3b8",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6}}>{"Role (optional)"}</div>
          <input style={{width:"100%",padding:"13px 14px",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,color:"#f1f5f9",fontSize:16,fontFamily:"inherit",outline:"none"}} placeholder="e.g. Bar Manager, Stage Crew" value={obRole} onChange={e=>setObRole(e.target.value)}/>
        </div>

        <div>
          <div style={{fontSize:11,fontWeight:800,color:"#94a3b8",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6}}>{"Phone Number *"}</div>
          <input type="tel" style={{width:"100%",padding:"13px 14px",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,color:"#f1f5f9",fontSize:16,fontFamily:"inherit",outline:"none"}} placeholder="6085551234" value={obPhone} onChange={e=>setObPhone(e.target.value)}/>
        </div>

        <button
          style={{padding:"16px",borderRadius:12,border:"none",background:(!obName.trim()||!obPhone.trim())?"rgba(255,255,255,0.06)":"linear-gradient(135deg,#10b981,#059669)",color:(!obName.trim()||!obPhone.trim())?"#64748b":"#fff",fontSize:16,fontWeight:800,cursor:(!obName.trim()||!obPhone.trim())?"not-allowed":"pointer",marginTop:4}}
    ...
    {/* ===== LOST & FOUND — SEPARATE SECTION ===== */}
      <div style={{background:"rgba(249,115,22,0.06)",borderRadius:14,border:"1px solid rgba(249,115,22,0.25)",overflow:"hidden"}}>
        ...
        <div style={{padding:"8px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
          <button style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",borderRadius:8,border:"1px solid rgba(249,115,22,0.2)",background:"rgba(249,115,22,0.06)",cursor:"pointer",textAlign:"left"}} onClick={()=>{setView("lostfound");fetchLFItems();}}>
          ...
          <div style={{padding:"4px 8px",display:"flex",flexDirection:"column",gap:3}}>
            {activityLog.slice(0,3).map(e=>(
              <div key={e.id} style={{fontSize:10,color:"#475569",display:"flex",gap:5}}>
                <span style={{color:"#e2e8f0",flexShrink:0}}>{e.ts}</span>
                <span style={{textOverflow:"ellipsis",overflow:"hidden",whiteSpace:"nowrap"}}>{e.label}</span>
              </div>
            ))}
            {activityLog.length===0&&<div style={{fontSize:11,color:"#e2e8f0",padding:"4px 0"}}>No activity yet</div>}
          </div>
        </div>
      </div>
    </div>
...
const S={
  ...
  bigAckBtn:{background:"rgba(255,255,255,0.04)",border:"2px solid rgba(255,255,255,0.7)",borderRadius:14,padding:"18px",color:"#fff",fontSize:18,fontWeight:900,cursor:"pointer",width:"100%",maxWidth:380},
